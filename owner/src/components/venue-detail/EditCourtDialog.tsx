        ...
        for (let i = 0; i < dayPricing.prices.length; i++) {
          ...
          if (minBookingMinutes && minBookingMinutes > 0) {
            ...
            if (startDiffMinutes % minBookingMinutes !== 0) {
              ...
              ctx.addIssue({
                code: z.ZodIssueCode.custom,
                message: `Giờ bắt đầu phải chia hết cho ${minBookingMinutes} phút tính từ giờ mở cửa ${formattedOpenTime}`,
                path: [`dailyPricing`, dayIndex, `prices`, i, `startTime`],
              });
            }
          }
...
interface EditCourtDialogProps {
  ...
  fieldsData: FieldOwnerResponse | null;
  updateCourtMutation: any;
  refetch: () => void;
}
...
export function EditCourtDialog({
  ...
  // Helper function to sort prices by start time
  const sortPricesByStartTime = (prices: any[]) => {
  ...
  // Prepare sorted daily pricing data
  const getSortedDailyPricing = (dailyPricing: any[]) => {
    return dailyPricing.map((day) => ({
      ...day,
      prices: sortPricesByStartTime(day.prices),
    }));
  };
  ...
  const {
    ...
    resolver: zodResolver(
      createCourtSchema(
        fieldsData?.openingHours || [],
        fieldsData?.minBookingMinutes || 0
      )
    ), // Initial resolver with empty data
    defaultValues: {
      name: editingCourt?.name || "",
      defaultPrice: editingCourt?.defaultPrice || 0,
      status: editingCourt?.status || "ENABLE",
      dailyPricing: editingCourt?.dailyPricing
        ? getSortedDailyPricing(editingCourt.dailyPricing)
        : [],
    },
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmitCourt(onSubmitCourt)}>
          <DialogHeader>
            ...
            <DialogDescription>
              Chỉnh sửa thông tin sân ở đây. Nhấn lưu khi hoàn tất.
            </DialogDescription>
          </DialogHeader>
                    ...
                    <div className="space-y-6 pr-2">
                      {dailyPricingFields
                        ...
                        .map((field, index) => {
                          ...
                          const openingHour = fieldsData?.openingHours.find(
                            (hour: any) => hour.dayOfWeek === field.dayOfWeek
                          );
